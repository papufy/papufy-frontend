...
export const ANUNCIAR_TYPE_CHOICES: Array<{
  type: ListingType;
  kicker: string;
  title: string;
  subtitle: string;
...
interface AnunciarTipoPickerProps {
  onSelect: (listingType: ListingType) => void;
  onBack?: () => void;
}

export function AnunciarTipoPicker({
  onSelect,
  onBack,
}: AnunciarTipoPickerProps) {
  ...
  return (
    <div className="mx-auto flex min-h-[100dvh] w-full max-w-lg flex-col overflow-x-hidden px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-[max(0.75rem,env(safe-area-inset-top))]">
      ...
      <div className="mt-5 flex flex-1 flex-col gap-3">
        {ANUNCIAR_TYPE_CHOICES.map((choice, index) => {
